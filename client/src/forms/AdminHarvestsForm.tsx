...
export const AdminHarvestsForm = () => {
  ...
  const stateToString = (state: string) =>
    t(
      'administration.harvests.states.' +
        (['UNPROCESSED', 'PROCESSING', 'INDEXED', 'ERROR', 'CLEARED'].includes(state)
          ? state
          : 'UNKNOWN')
    );
  ...
  return (
    <>
      <Typography variant="h1">{t<string>('administration.harvests.title')}</Typography>
      <Card variant="outlined">
        <TableContainer>
          <Table>
            <TableHead>
              <TableRow>
                <TableCell>{t<string>('administration.harvests.columns.id')}</TableCell>
                <TableCell>{t<string>('harvests.date')}</TableCell>
                <TableCell>{t<string>('administration.harvests.columns.type')}</TableCell>
                <TableCell>{t<string>('query.state')}</TableCell>
                <TableCell>
                  {t<string>('administration.harvests.columns.numberOfRecords')}
                </TableCell>
                <TableCell />
              </TableRow>
            </TableHead>
                    ...
                    {!['UNPROCESSED', 'PROCESSING', 'INDEXED'].includes(row.state) && (
                      <Button
                        ...
                        onClick={() => {
                          ...
                          fetch('/api/harvest/index?harvestId=' + row.identification, {
                            method: 'POST'
                          })
                            .then(() => {
                              refreshHarvests();
                        ...
                        {t<string>('administration.harvests.actions.import')}
                      </Button>
                    )}
                    {row.state === 'INDEXED' && (
                      <Button
                        ...
                        onClick={() => {
                          ...
                          fetch('/api/harvest/clear?harvestId=' + row.identification, {
                            method: 'POST'
                          })
                            .then(() => {
                              refreshHarvests();
                        ...
                        {t<string>('administration.harvests.actions.delete')}
                      </Button>
                    )}
                    {['UNPROCESSED', 'PROCESSING'].includes(row.state) && (
      ...
      <Box my={4}>
        <Button
          ...
          onClick={() => {
            ...
            fetch('/api/harvest/clear-all', { method: 'POST' })
              .then(() => {
                refreshHarvests();
                addNotification(
          ...
          {t<string>('administration.harvests.actions.deleteAll')}
        </Button>
      </Box>
    </>
  );
