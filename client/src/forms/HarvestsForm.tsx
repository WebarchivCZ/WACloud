...
import {Grid, Card, CardContent, Typography, makeStyles, Checkbox, CardHeader, Box} from "@material-ui/core";
...
const fakeData: FakeDataProps[] = [
  ...
  {name: 'Topics_2021-07_T-CUNI_MGHMP_Klima_Tornado2_Olympiada3', size: '260 GB', warcs: 236, date: '2021-07-30'},
...
export const HarvestsForm = ({harvests, setHarvests, minimal}:HarvestsProps) => {
  ...
  const handleToggleHarvest = (name: string) => () => {
    let harvestsCopy = _.clone(harvests);
  ...
  useEffect(() => {
    fetch("/api/harvest")
      ...
      .then(
        (result) => {
          let harvests = result.filter((h: IHarvest) => h.state === 'INDEXED');
// TODO FAKE REMOVE start
          while (harvests.length < fakeData.length) {
            let fake = fakeData[harvests.length];
            harvests.push({
  ...
  return (
    <Grid container spacing={2}>
      <Grid item xs={12}>
        ...
        {minimal && allHarvests.filter(h => isHarvestChecked(h.identification)).map((harvest, index) => (
      ...
      {!minimal && allHarvests.map((harvest, index) => (
        <Grid item xs={12} sm={12} md={6} lg={4} xl={3} key={index}>
          <Card variant="outlined" className={clsx({
            ...
            <CardContent className={classes.content}>
              <table>
                <tbody>
                <tr><th>{t('harvests.name')}</th><td>{harvest.identification}</td></tr>
                <tr><th>{t('harvests.size')}</th><td>{fakeData[index].size}</td></tr>
                <tr><th>{t('harvests.WARCs')}</th><td>{harvest.entries.toLocaleString(i18n.language)}</td></tr>
                <tr><th>{t('harvests.date')}</th><td>{(new Date(harvest.date)).toLocaleDateString(i18n.language)}</td></tr>
                </tbody>
              </table>
            </CardContent>
          </Card>
